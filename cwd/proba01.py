...
def news(newstext):
    #a=soup.find('a', text=re.compile('Средние цены на отдельные потребительские товары'))
...
def newsin(file_name, textin):    
    if os.path.exists(file_name)==True:
        ...
        #for link in soup.findAll('a',text=re.compile('Ненецкому автономному округу')):
        for link in soup.findAll('a',text=re.compile(textin)):
            tovnao='https://arhangelskstat.gks.ru'+link.get('href')
        #Добавляем headers, т.к. получали ошибку "TimeoutError: [WinError 10060]" при попытке скачать файл
...
def tabl(n,m):
    for j in range(1,n):
        try:
            ...
            print(f'j={j}, j-1: {table.cell(j-1,0).text}={table.cell(j-1,1).text}')
            sys.exit()
        ...
        if cell>t[j-1]:
            ...
            table.cell(j,2).text_frame.paragraphs[0].font.color.rgb = RGBColor(79, 98, 40) #Green
        elif cell<t[j-1]:
            table.cell(j,2).text=rup
            table.cell(j,2).text_frame.paragraphs[0].font.size = Pt(16)
            table.cell(j,2).text_frame.paragraphs[0].font.bold = True
            table.cell(j,2).text_frame.paragraphs[0].font.color.rgb = RGBColor(158, 0, 0) #Red
        else:
            table.cell(j,2).text=''
            table.cell(j,2).text_frame.paragraphs[0].font.size = Pt(16)
...
#Добавляем headers, т.к. получали ошибку при 1ом вызове news() через присваивание к alink
...
path2019='d:/code/python/zem/price/2019/'
fnamexls=path2019+f'arhangelskstat_{datasite}.xls'
...
if datasite_int==datasiteai_int:
    fnamedoc=path2019+f'arhangelskstat_AI92_{datasiteai}.doc'
    newsin(fnamedoc,'Ненецком автономном округе')
else:
    print('Внимание! Проверьте даты новостей с ценами на бензин и др. товары')
#-----------------------------------------------
...
for i in range(sheet.nrows):
    for string in tovar:
        ...
        if match:
            ...
            #print(f'idx={tovar.index(string)},  str=\'{string}\',   {sheet.cell_value(i, 0)}={sheet.cell_value(i, 1)}')
#-----------------------------------------------

#DOC файл с ценами на бензин АИ-92
if os.path.exists(fnamedoc+'x')==False:    
    doc2docx(path2019)
...
#PPTX
location = 'D:/code/python/zem/price/midprice.pptx'
prs = Presentation(location)
prsoutf = 'D:/code/python/zem/price/midpriceOut.pptx'
rup = '↑'
rdo = '↓'
