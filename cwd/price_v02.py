...
def dat(txt):
    ...
    # dt = re.compile("\d+( |.)([яфмаисонд](([а-я]+[а|я] ))\d{4})")
...
if not os.path.exists(path2019):
    os.mkdir(path2019)
xl_file = path2019+f'arhangelskstat_{datasite}.xls'  # XLSX path
...
'''
# -----------------------------------------------
# РАБОТА С ФАЙЛАМИ БЕЗ ЗАКАЧКИ
path2019 = katalog + '/2019/'
xl_file = path2019+f'arhangelskstat_20191028.xls'  # XLSX path
fnamedoc = path2019+f'arhangelskstat_AI92_20191028.doc'
new_date = '28 октября 2019'
# -----------------------------------------------
'''
t = []
...
#pptx_out = katalog + f'/midPriceOut_20191028.pptx'
